% 18052022 change line 26 for i=1:10; line 60-66, u_cl update
% y_pred, use same N_init (paper version) D ~= 0
clc
clear
rng(353) % 353 126: tzero(A,B,C,zeros(ny,nu),eye(nx))>1
%%
% system definition
% A = [1 1; 0 1];
% B = [1; 1];%[0.5; 1]; 
...
% [U,S,V] = svd(tem3);
% % G1 = V*[inv(S(1:16,1:16)) rand(16,6);rand(29,22)]*U';
% G1 = V*[inv(S(1:15,1:15)) zeros(15,7);zeros(30,22)]*U';
% % G1 = pinv(tem3);
% % G1 = V*[inv(S(1:22,1:22)) ;zeros(23,22)]*U';
...
% use N_init(paper):  
% U3*u_vector that is U3*z_{t-1}
% Y3*[yi;yp] that is Y3*d_{t-1}
UY3 = Hankel_Y(ny*(N_ini)+1:ny*(N_ini+1), :)*pinv([Hankel_U(1:nu*(N_ini+1), :); Hankel_Y(1:ny*(N_ini), :);]);
U3 = UY3(:,1:nu*N_ini) + UY3(:,nu*N_ini+1:nu*(N_ini+1))*U1;
Y3 = [UY3(:,nu*(N_ini+1)+1:end) zeros(ny, ny)] + UY3(:,nu*N_ini+1:nu*(N_ini+1))*Y1;
...
u_vector = reshape(u_pred_basic(:,t-N_ini:t-1), [N_ini*nu,1]);
...
%     %     [Hankel_U;Hankel_y_past]*g-[u_ini_pred;y_ini] % Almost 0
        tem = Hankel_U_future*g;
        % U_check: initial A_UIE; some error feedback
        u_vector =  U_check*u_vector + [zeros(nu*(N_ini-1),size(Y1,2)); Y1]*[yi;yp] + K'*(yp - Y3*[yi;yp] - U3*u_vector);
        u_pred_basic(:,t:t+N_pred-1) = u_vector(end-nu+1:end,1);
...
%%
start = T+1-5; last = size(u_pred_basic,2);
u1_p2 = u_pred_basic(:,start:last); % u1: D~=0; p2: Luen u
u1_r2 = u_cl(:,start:last); % r2: real u
save('./data_for_latex/u1_p2.mat','u1_p2','u1_r2');

%%
tzero(A,B,C,D,eye(nx))
%%
