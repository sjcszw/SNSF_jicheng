...
clc
clear
rng(127) % 126: tzero(A,B,C,zeros(ny,nu),eye(nx))>1
%%
% system definition
% A = [1 1; 0 1];
% B = [1; 1];%[0.5; 1]; 
...
nx = size(A, 1);
ny = size(C, 1);
D = 2*rand(2,2); %[1 0; 1 0];%zeros(ny,nu); 2*rand(2,2);
...
[U,S,V] = svd(tem3);
% G1 = V*[inv(S(1:18,1:18)) zeros(18,6);zeros(27,24)]*U';
G1 = V*[inv(S(1:24,1:24)) ;zeros(21,24)]*U';
...
figure()
hold on; grid on;
start = T+1; last = size(u_pred_basic,2)-45;
