...
ny = 1;
%%
load('clear_06-12-2021_19-12-2021.mat')
N_ini = 10;
N_pred = 1;

y_cl = mean([exp.co2_1,exp.co2_2,exp.co2_3,exp.co2_4],2)'/100;
% y_cl = [exp.co2_1,exp.co2_2,exp.co2_3,exp.co2_4]'/100;
% difference 
    % y_cl = [y_cl(:,1:end-1);y_cl(:,2:end)-y_cl(:,1:end-1)];

% filter
    % tem1 = 4;
    % tem2 = [y_cl, zeros(ny,tem1-1)];
    % for i = 1:size(y_cl,2)
    %     tem3(:,i) = tem2(:,i:i+tem1-1)*ones(tem1,1)/tem1;
    % end
    % tem1 = y_cl;
    % y_cl = tem3;
% detrend
    % y_cl_env = mean([exp.co2_1,exp.co2_2,exp.co2_3,exp.co2_4]');
    % y_cl_env = [y_cl_env(:,96*7+1:96*7+32), y_cl_env(:,96*6+33:96*7)];
    % y_cl_env = kron(ones(ny,9),y_cl_env);
    % y_cl_env = y_cl_env(1:size(y_cl,2))/100;
    % y_cl = y_cl - y_cl_env;
u_cl = exp.people';
% not use 1st day
    % y_cl = y_cl(:,97:end);
