...
% At time t, use data t + N_pred to estimate occ, N_pred > 1
clc
clear
rng(125)
%%
...
% temp + co2: N_pred =4; N_ini=10 raw 7.4807,  N_ini=4  7.4919
%       N_ini=5 raw 7.5056  
% temp + co2: N_pred =2; N_ini=10 raw 6.1634,  N_ini=4  5.6972
%       N_ini=5 raw  5.5686

% Calibration at night; start = 7*96+1; last = size(u_pred,2)-96*2;
% temp + co2: N_pred =2; N_ini=10 raw 4.6409,  N_ini=4  4.5985
%       N_ini=5 raw  4.4843
% temp + co2: N_pred =3;  N_ini=5 raw  5.3540

% Calibration at night; start = 7*96+1; last = 12*96;
% temp + co2: N_pred =1;  N_ini=5 raw  4.3402
% temp + co2: N_pred =2;  N_ini=5 raw  4.3461
% temp + co2: N_pred =3;  N_ini=5 raw  5.1096
...
N_total = N_ini + N_pred_people;
T_people = Hankel_col + N_total -1;% need same Hankel_col
Hankel_U = compute_Hankel_matrix(u_cl(:,1:T_people),nu,N_total, Hankel_col);
...
%         u_pred(:,t:t+N_pred_people-1) = reshape(Hg_pred*[yi;ui;yp;wip],[nu, N_pred_people]);
%     else
%         break;
%     end
% end
% calibrate at night
for i = 7:14
    for t = i*96+7*4+1:N_pred_people:(i+1)*96
...
function Hg_pred = Hg_problem_u(Hankel_U,Hankel_Y,Hankel_W,nu,ny,N_ini,Qg)
    % u_pred = Hg_pred*[y_ini;u_ini;y_pred;w_ini;w_pred];
            Hy_init = Hankel_Y(1:ny*(N_ini+1), :);
            Hy_pred = Hankel_Y(ny*(N_ini+1)+1:end, :); 
