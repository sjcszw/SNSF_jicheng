% y_pred, use same N_init (paper version) In the old one, use y_{t} =
% ...z{t-1} + ...d{t-1} already one more step
% D = 0
clc
clear
...
% use N_init(paper):  This file: yi use N_init+1 steps
...
Y3 = [UY3(:,nu*(N_ini+1)+1:end) zeros(ny, 2*ny)] + UY3(:,nu*N_ini+1:nu*(N_ini+1))*Y1;
...
%         wip = reshape(w_cl(:,t-N_ini:t+N_pred-1), [(N_ini+N_pred)*nw,1]);
        ...
        u_vector =  U_check*u_vector + [zeros(nu*(N_ini-1),size(Y1,2)); Y1]*[yi;yp] + K'*(yi(end-ny+1:end,:)-Y3*[yi;yp] - U3*u_vector);
...
save('./data_for_latex/u2_p2.mat','u2_p2','u2_r2');
%%
