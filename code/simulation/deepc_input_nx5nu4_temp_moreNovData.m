...
load('clear_2021-11-12_2021-11-23.mat')
N_ini = 10;
N_pred = 1;

y_hankel = mean([exp_hankel.sensor_temp_1,exp_hankel.sensor_temp_2,exp_hankel.sensor_temp_3,exp_hankel.sensor_temp_4],2)';
u_hankel = exp_hankel.people';
w_hankel = [exp_hankel.power-2.35,exp_hankel.weather_temp,exp_hankel.weather_rad/100]';
...
T = 96*7; % T >= (nu + 1)*(N_ini + N_pred + nx) - 1
...
Hankel_U = compute_Hankel_matrix(u_hankel(:,1:T),nu,N_total, Hankel_col);
Hankel_W = compute_Hankel_matrix(w_hankel(:,1:T),nw,N_total, Hankel_col);
% Hankel_W_elec = compute_Hankel_matrix(exp_elec_iden.w,nw,N_total, Hankel_col);

N_total = N_ini + N_pred + 1;
Hankel_col = T - N_total + 2;
Hankel_Y = compute_Hankel_matrix(y_hankel(:,1:T+1),ny,N_total, Hankel_col);
...
Hankel_UW_check = compute_Hankel_matrix([u_hankel(:,1:T);w_hankel(:,1:T)], nu+nw, N_total, Hankel_col);
...
for i = 1:14
    for t = i*96+7*4+1:N_pred:(i+1)*96
        if t+N_pred<=(i+1)*96 && t+N_pred<=T_tot
...
figure()
hold on; grid on;
start = 1*96+1; last = size(u_pred,2);
plot( u_cl(num,start:last), 'Color',[0 0.4470 0.7410],'LineWidth',1)
plot( u_pred(num,start:last), 'Color',[0.8500 0.3250 0.0980],'LineWidth',1)
...
legend('u real','u bi-level','prediction point','FontSize',18)
